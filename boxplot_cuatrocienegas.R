...
dir.create(path = "data_4cienegas")
...
worldclim_fxn <- function(model, time_range, ssp) {
  ...
  path=paste("data_4cienegas/",model) #name of paths 
...
forecast_2021<-forecast_2021[,]-bioclim_1970[,2]; forecast_2021$year<-"2021-2040"; forecast_2021$ubi<-c("Poza Churince", "Poza Azul", "Las Playitas")
forecast_2041<-forecast_2041[,]-bioclim_1970[,2]; forecast_2041$year<-"2041-2060"; forecast_2041$ubi<-c("Poza Churince", "Poza Azul", "Las Playitas")
forecast_2061<-forecast_2061[,]-bioclim_1970[,2]; forecast_2061$year<-"2061-2080"; forecast_2061$ubi<-c("Poza Churince", "Poza Azul", "Las Playitas")
...
forecast_all <- melt(forecast_all, id.vars= c("year","ubi")) 

plot<-ggplot(forecast_all, aes(year, value)) + geom_boxplot() +
  ...
  Poza Churince, Poza Azul and Las Playitas, located in Cuatro Ciénegas, Coahuila, Mexico. Temperature values were extracted from 25 CMIP-6 models available at: 
  https://www.carbonbrief.org/cmip6-the-next-generation-of-climate-models-explained/")


ggsave(plot, file="plot_cuatrocienegas.png", width = 25, height = 14, units = "cm")
